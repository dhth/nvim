...
local function failed_test_details(inputstr, projectstr)
    local file_name = SPLIT(SPLIT(inputstr, " ")[2], "::")[1]
    ...
    local function_name = SPLIT(
        SPLIT(SPLIT(inputstr, "::")[#SPLIT(inputstr, "::")], " ")[1],
        "["
    )[1]
    return {
...
local function run_tests_via_vimux(opts)
    local config = READ_FILE_LINES "./testconfig"
    pickers
        .new(opts, {
            prompt_title = "colors",
    ...
    vim.fn.jobstart(SPLIT(cmd, " "), {
        ...
        on_stdout = function(_, data)
            for _, line in ipairs(data) do
                if string.match(line, "Spec.scala:") then
                    test_details_raw = TRIM(
                        vim.fn.system(
                            "echo '"
                                .. line
                                .. "' | python $HOME/.config/nvim/lua/dhth/command_to_buffer/helper.py"
                        )
                    )
        ...
        on_exit = function()
            ...
            for _, details in ipairs(state) do
                full_file_path = TRIM(
                    vim.fn.system(
                        "fd -ipH -t f " .. details.file_name .. ".scala"
                    )
                )
                failed[#failed + 1] = {
            ...
            if #state > 0 then
                vim.fn.setqflist(
                    {},
                    "r",
                    { title = "Test failures ❌ ", items = failed }
                )
...
function M.failed_test_qf()
    ...
    elseif file_type == "python" then
        local last_test_project = READ_FILE_LINES("testlastproject")[1][1]
...
function M.search_tests_for_current_file()
    ...
    if file_type == "python" then
        require("dhth.telescope").find_test_files(
...
REMAP("n", "<Leader>st", M.search_tests_for_current_file, OPTS_NO_REMAP_SILENT)

--- Go to next test
NOREMAP_SILENT("n", "<Leader>nt", M.go_to_next_test)

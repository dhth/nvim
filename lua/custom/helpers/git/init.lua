...
local function create_telescope_search(opts)
    ...
    local config = {
        ...
        {
            "diff window",
            M.show_diff_in_window,
            "0",
            "",
        },
        {
            "diff branch",
            M.show_diff_for_branch,
            "0",
            "",
        },
        {
        ...
        {
            "stash push, rebase o/master and stash pop",
            "Git stash push | Git rebase origin/master | Git stash pop",
            "0",
            "rebased!",
        },
        {
            "stash push, checkout master, pull, and stash pop",
            "Git stash push | Git checkout master | Git pull | Git stash pop",
            "0",
            "checked out master!",
        },
    }
        ...
        .new(opts, {
            ...
            attach_mappings = function(prompt_bufnr, _)
                actions.select_default:replace(function()
                    ...
                    local selection = action_state.get_selected_entry()
                    local commit_hash = SPLIT_STRING(
                        SPLIT_STRING(selection.value, ">> ")[2],
                        " <<"
                    )[1]
...
function M.git_diff()
    local commits =
        vim.fn.systemlist "git --no-pager log --all --graph --oneline --pretty=format:'      >> %h <<   %d %s (%cr)' --since='1 month ago'"
...
function M.show_diff_for_branch()
    ...
    vim.fn.inputrestore()
    if target_branch == "q" then
        print " quitting.."
        return
    end
    local cmd = "DiffviewOpen origin/"
        .. base_branch
        .. "..origin/"
        .. target_branch
    vim.fn.inputsave()
...
function M.diff_with_main_branch()
    local main_branch = TRIM(
        vim.fn.system '[ -f ".mainbranch" ] && cat ".mainbranch" || echo "main"'
    )
...
--- git helpers
NOREMAP_SILENT("n", "<Leader>gh", M.git_commands)

--- git push
NOREMAP_SILENT("n", "<Leader>gp", M.git_push)

--- diff file with main branch
NOREMAP_SILENT("n", "<Leader>dm", M.diff_with_main_branch)

--- diff file
NOREMAP_SILENT("n", "<leader>df", M.diff_file)

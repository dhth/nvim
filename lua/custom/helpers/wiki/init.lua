...
function M.toggle_line_checklist()
    ...
    if vim.fn.match(current_line, "- \\[ \\]") ~= -1 then
        table.insert(
            changed_lines,
            vim.fn.substitute(current_line, "- \\[ \\]", "- \\[x\\]", "")
        )
    elseif vim.fn.match(current_line, "- \\[x\\]") ~= -1 then
        table.insert(
            changed_lines,
            vim.fn.substitute(current_line, "- \\[x\\]", "- \\[ \\]", "")
        )
...
function M.open_urls()
    ...
    local selected_lines =
        vim.api.nvim_buf_get_lines(0, start_line - 1, end_line, true)
    for _, line in ipairs(selected_lines) do
        if vim.fn.match(line, "https://") ~= -1 then
...
function M.toggle_visual_checklist()
    ...
    for _, line in ipairs(selected_lines) do
        if vim.fn.match(line, "- \\[ \\]") ~= -1 then
            table.insert(
                changed_lines,
                vim.fn.substitute(line, "- \\[ \\]", "- \\[x\\]", "")
            )
        elseif vim.fn.match(line, "- \\[x\\]") ~= -1 then
            table.insert(
                changed_lines,
                vim.fn.substitute(line, "- \\[x\\]", "- \\[ \\]", "")
            )
